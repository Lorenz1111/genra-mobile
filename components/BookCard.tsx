...
export const BookCard = ({
  ...
  const isNew =
    new Date(book.created_at).getTime() > Date.now() - 7 * 24 * 60 * 60 * 1000;

  const chapterNumbers =
    book.chapters?.map((c: any) => c.sequence_number) || [];
  const latestChapter =
    chapterNumbers.length > 0 ? Math.max(...chapterNumbers) : 0;

  const validCover =
    book.cover_url && book.cover_url.trim() !== "" && book.cover_url !== "null"
      ? book.cover_url
  ...
  return (
    <Pressable
      ...
      style={[
        variant === "grid"
          ? { flex: 1, maxWidth: "48%", marginBottom: 24 }
          : { width: 128, marginRight: 16 },
        { overflow: "visible" },
      ]}
    >
