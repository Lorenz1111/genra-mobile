...
export const CommentSection = ({ bookId }: { bookId: string }) => {
  ...
  const fetchComments = useCallback(async () => {
    const { data, error } = await supabase
      .from("book_comments")
      .select(
        `*, profiles(full_name, avatar_url, is_verified), comment_votes(user_id, vote_type)`,
      )
      .eq("book_id", bookId)
  ...
  const handlePostComment = async (
    ...
    if (!currentUserId)
      return Alert.alert("Notice", "You need to log in to post.");
    ...
    if (error) {
      ...
      if (parentId) {
        ...
        setReplyText("");
        setShownReplies((prev) => ({ ...prev, [parentId]: true }));
      } else {
        setNewComment("");
      }
      Keyboard.dismiss();
  ...
  const openActionSheet = (commentId: string, isMyComment: boolean) => {
    setSelectedComment({ id: commentId, isMyComment });
  ...
  const handleActionSheetAction = async (action: "delete" | "report") => {
    ...
    setTimeout(async () => {
      if (action === "delete") {
        ...
        Alert.alert(
          "Reported",
          "Thanks for keeping our community safe. Our team will review this comment.",
        );
      }
    }, 300);
  };
  ...
  const handleVote = async (commentId: string, type: "like" | "dislike") => {
    ...
    if (existingVote && existingVote.vote_type === type) {
      ...
      await supabase
        .from("comment_votes")
        .upsert(
          { comment_id: commentId, user_id: currentUserId, vote_type: type },
  ...
  const CommentAvatar = ({
    ...
    if (url && url !== "null" && url.trim() !== "")
      return (
        <Image
          source={{ uri: url }}
          className={`${size} rounded-full bg-slate-200 border border-slate-100`}
        />
      );
    return (
      <View
        className={`${size} rounded-full bg-slate-100 items-center justify-center border border-slate-200`}
      >
  ...
  return (
    <View className="pb-10 relative">
      ...
      <View className="flex-row items-center bg-slate-100 rounded-3xl pl-4 pr-1.5 py-1 border border-slate-200 mb-8 shadow-sm">
        ...
        />
        <Pressable
          onPress={() => handlePostComment(null)}
          disabled={submitting || !newComment.trim()}
          className="w-10 h-10 rounded-full items-center justify-center ml-1"
          style={{
            backgroundColor: newComment.trim() ? "#2563EB" : "transparent",
          }}
        >
          {submitting && !replyingToId ? (
            ...
            <Ionicons
              ...
              style={newComment.trim() ? { marginLeft: 2 } : {}}
            />
          )}
        </Pressable>
      </View>
      ...
      {displayedMainComments.length === 0 ? (
        ...
        displayedMainComments.map((mainComment) => {
          ...
          const mainLikeCount = mainVotes.filter(
            (v) => v.vote_type === "like",
          ).length;
          const mainDislikeCount = mainVotes.filter(
            (v) => v.vote_type === "dislike",
          ).length;
          const myMainVote = mainVotes.find(
            (v) => v.user_id === currentUserId,
          )?.vote_type;
          ...
          return (
            <View key={mainComment.id} className="mb-6">
              <Pressable
                onLongPress={() =>
                  openActionSheet(mainComment.id, isMyMainComment)
                }
                delayLongPress={300}
                className="flex-row items-start active:opacity-80 mb-2"
              >
                <View className="mr-2 mt-0.5">
                  <CommentAvatar
                    url={mainComment.profiles?.avatar_url || null}
                  />
                </View>
                <View className="flex-1">
                  <View className="bg-slate-50 px-4 py-3 rounded-2xl rounded-tl-sm border border-slate-100">
                    <View className="flex-row items-center gap-1.5 mb-1">
                      <Text className="font-bold text-slate-900 text-[13px]">
                        {mainComment.profiles?.full_name || "Anonymous Reader"}
                      </Text>
                      {mainComment.profiles?.is_verified && (
                        <Ionicons
                      ...
                      <Text className="text-[11px] text-slate-400 font-medium ml-1">
                        {timeAgo(mainComment.created_at)}
                      </Text>
                    </View>
                    <Text className="text-slate-800 text-[15px] leading-5">
                      {mainComment.comment_text}
                    </Text>
                  </View>
                  <View className="flex-row items-center gap-5 mt-1.5 ml-2">
                    <Pressable
                      onPress={() => handleVote(mainComment.id, "like")}
                    ...
                    >
                      <Ionicons
                        ...
                        size={14}
                        color={myMainVote === "like" ? "#2563EB" : "#94a3b8"}
                      />
                      <Text
                        className={`text-xs font-bold ${myMainVote === "like" ? "text-blue-600" : "text-slate-500"}`}
                      >
                        {mainLikeCount}
                      </Text>
                    </Pressable>
                    <Pressable
                      onPress={() => handleVote(mainComment.id, "dislike")}
                      className="flex-row items-center gap-1 active:opacity-50"
                    >
                      <Ionicons
                        name={
                        ...
                        size={14}
                        color={myMainVote === "dislike" ? "#ef4444" : "#94a3b8"}
                      />
                      <Text
                        className={`text-xs font-bold ${myMainVote === "dislike" ? "text-red-500" : "text-slate-500"}`}
                      >
                        {mainDislikeCount}
                      </Text>
                    </Pressable>
                    <Pressable
                      onPress={() => {
                        setReplyingToId(
                          isReplyingInline ? null : mainComment.id,
                        );
                        setReplyText("");
                        if (!isReplyingInline)
                ...
                <View className="ml-[48px]">
                  {replies.length > 0 && !showReplies && (
                    <Pressable
                      ...
                      className="mb-3 py-1 active:opacity-60"
                    >
                  ...
                  {showReplies &&
                    replies.map((reply) => {
                      ...
                      const replyLikeCount = replyVotes.filter(
                        (v) => v.vote_type === "like",
                      ).length;
                      const replyDislikeCount = replyVotes.filter(
                        (v) => v.vote_type === "dislike",
                      ).length;
                      const myReplyVote = replyVotes.find(
                        (v) => v.user_id === currentUserId,
                      )?.vote_type;

                      let replyName = "";
                      let actualText = reply.comment_text;
                      if (actualText.startsWith("@")) {
                        const spaceIdx = actualText.indexOf(" ");
                        if (spaceIdx !== -1) {
                          replyName = actualText.substring(0, spaceIdx);
                          actualText = actualText.substring(spaceIdx + 1);
                        }
                      }
                      ...
                      return (
                        <Pressable
                          key={reply.id}
                          onLongPress={() =>
                            openActionSheet(reply.id, isMyReply)
                          }
                          delayLongPress={300}
                          className="flex-row items-start mb-2 active:opacity-80"
                        >
                          <View className="mr-2 mt-0.5">
                            <CommentAvatar
                          ...
                          <View className="flex-1">
                            ...
                            <View className="flex-row items-center gap-4 mt-1.5 ml-2">
                              <Pressable
                                onPress={() => handleVote(reply.id, "like")}
                                className="flex-row items-center gap-1 active:opacity-50"
                              >
                                <Ionicons
                                  name={
                                ...
                                />
                                <Text
                                  className={`text-[11px] font-bold ${myReplyVote === "like" ? "text-blue-600" : "text-slate-500"}`}
                                >
                                  {replyLikeCount}
                                </Text>
                              </Pressable>
                              <Pressable
                                onPress={() => handleVote(reply.id, "dislike")}
                                className="flex-row items-center gap-1 active:opacity-50"
                              >
                                <Ionicons
                                  name={
                                ...
                                />
                                <Text
                                  className={`text-[11px] font-bold ${myReplyVote === "dislike" ? "text-red-500" : "text-slate-500"}`}
                                >
                                  {replyDislikeCount}
                                </Text>
                              </Pressable>
                              <Pressable
                                onPress={() => {
                                  setReplyingToId(mainComment.id);
                                  setReplyText(
                                    `@${reply.profiles?.full_name?.split(" ")[0]} `,
                                  );
                                  setTimeout(
                  ...
                  {isReplyingInline && (
                    <View className="flex-row items-center mt-2 mb-2">
                      <TextInput
                        ref={replyInputRef}
                        className="flex-1 bg-white border border-blue-200 text-slate-800 py-1.5 px-3 text-[15px] leading-5 rounded-full"
                        placeholder={`Reply to ${mainComment.profiles?.full_name?.split(" ")[0]}...`}
                        placeholderTextColor="#94a3b8"
                        value={replyText}
                      ...
                      <Pressable
                        onPress={() =>
                          handlePostComment(
                            mainComment.id,
                            mainComment.profiles?.full_name || "User",
                          )
                        }
                        disabled={submitting || !replyText.trim()}
                        className="w-9 h-9 ml-2 justify-center items-center rounded-full bg-blue-600"
                        style={{
                      ...
                      >
                        <Ionicons
                          ...
                          style={replyText.trim() ? { marginLeft: 2 } : {}}
                        />
                      </Pressable>
                    </View>
                  )}
      ...
      {visibleCount < mainComments.length && (
        <Pressable
          ...
          className="py-3 items-center justify-center bg-slate-50 rounded-2xl border border-slate-200 active:bg-slate-100 mt-2 shadow-sm"
        >
      ...
      >
        <View className="flex-1 justify-end bg-black/40">
          ...
          <View className="bg-white rounded-t-[32px] p-6 pb-12 shadow-2xl z-10">
            ...
            <Text className="text-center text-slate-400 font-black text-xs uppercase tracking-widest mb-4">
              Comment Options
            </Text>
            {selectedComment?.isMyComment ? (
              <Pressable
                onPress={() => handleActionSheetAction("delete")}
                className="flex-row items-center p-4 bg-red-50 rounded-2xl mb-3 active:opacity-70"
              >
              ...
              <Pressable
                ...
                className="flex-row items-center p-4 bg-slate-50 border border-slate-100 rounded-2xl mb-3 active:opacity-70"
              >
                <View className="w-10 h-10 bg-slate-200 rounded-full items-center justify-center mr-4">
                  <Ionicons name="flag" size={20} color="#475569" />
                </View>
            ...
            <Pressable
              onPress={() => setActionSheetVisible(false)}
              className="items-center py-4 mt-2 active:bg-slate-50 rounded-2xl"
            >
