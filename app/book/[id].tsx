...
export default function BookDetails() {
  ...
  const handleRateBook = async (stars: number) => {
    ...
    if (!user || !book)
      return Alert.alert("Notice", "You must be logged in to rate a book.");

    setUserRating(stars);
    const { error } = await supabase
      .from("book_ratings")
      .upsert({ user_id: user.id, book_id: book.id, rating: stars });
  ...
  const toggleBookmark = async () => {
    ...
    if (!user || !book)
      return Alert.alert("Notice", "You must be logged in to bookmark.");
  ...
  const handleShare = async () => {
    ...
    try {
      // SENIOR DEV FIX: Ginamit ang app scheme mo para sa Deep Link support
      const shareUrl = `genramobile://book/${book.id}`;
      const webUrl = `https://genra.app/book/${book.id}`; // Fallback web link

      await Share.share({
        message: `Check out this awesome story I'm reading on GenrA: "${book.title}" by ${book.profiles?.full_name || "Unknown Author"}!\n\nRead it here: ${webUrl}`,
        title: book.title,
        url: shareUrl, // Para sa iOS support
      });
    } catch (error) {
      Alert.alert(
        "Share Failed",
        "Could not open the share menu at this time.",
      );
    }
  };
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () =>
            Alert.alert(
              "Coming Soon",
              "The Coin Top-up and Unlocking system is currently under development.",
            ),
        },
      ],
    );
  ...
  if (!book)
    return (
      <Text className="mt-10 text-center font-bold text-slate-500">
        Book not found
      </Text>
    );
  ...
  const validCover =
    book.cover_url ||
    "https://placehold.co/400x600/ffffff/94a3b8.png?text=No+Cover";
    ...
    <View className="flex-1 bg-white">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="absolute top-12 left-0 right-0 z-20 flex-row justify-between px-6">
          <Pressable
            onPress={() => router.back()}
            className="bg-black/30 p-2.5 rounded-full active:bg-black/50"
          >
        ...
        <View
          className="px-6 -mt-32 pb-32 bg-white rounded-t-[40px] min-h-screen shadow-xl z-10 relative"
          style={{
            shadowColor: "#000",
        ...
        >
          <View className="flex-row justify-between items-start pt-8">
            <View className="flex-1 mr-4">
              ...
              <Text className="text-base text-slate-500 font-medium mt-1">
                by {book.profiles?.full_name || "Unknown Author"}
              </Text>
            </View>

            <Pressable
              onPress={toggleBookmark}
              className="p-3 bg-slate-50 rounded-full border border-slate-100 shadow-sm active:bg-slate-100"
            >
              <Ionicons
          ...
          <View className="flex-row flex-wrap gap-2 mt-4">
            {genres.map((genre: string, idx: number) => (
              <View
                key={idx}
                className="bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100"
              >
          ...
          <View className="flex-row border-b border-slate-200 mt-8 mb-6">
            <Pressable
              ...
              className={`flex-1 pb-3 items-center border-b-2 ${activeTab === "about" ? "border-primary" : "border-transparent"}`}
            >
              <Text
                className={`font-bold ${activeTab === "about" ? "text-primary" : "text-slate-400"}`}
              >
                About
              </Text>
            </Pressable>
            <Pressable
              ...
              className={`flex-1 pb-3 items-center border-b-2 ${activeTab === "chapters" ? "border-primary" : "border-transparent"}`}
            >
              <Text
                className={`font-bold ${activeTab === "chapters" ? "text-primary" : "text-slate-400"}`}
              >
                Chapters
              </Text>
            </Pressable>
            <Pressable
              ...
              className={`flex-1 pb-3 items-center border-b-2 ${activeTab === "comments" ? "border-primary" : "border-transparent"}`}
            >
              <Text
                className={`font-bold ${activeTab === "comments" ? "text-primary" : "text-slate-400"}`}
              >
                Comments
              </Text>
            </Pressable>
          ...
          {activeTab === "about" && (
            <View>
              ...
              <Text
                className="text-slate-600 leading-6 text-justify"
              ...
              {(book.description?.length ?? 0) > 150 && (
                <Pressable
                  ...
                  className="mt-2 self-start active:opacity-50"
                >
          ...
          {activeTab === "chapters" && (
            <View>
              ...
              {chapters.length === 0 ? (
                <Text className="text-center text-slate-400 italic mt-6">
                ...
                sortedChapters.map((chapter) => {
                  ...
                  return (
                    ...
                    >
                      <View
                        className={`w-10 h-10 rounded-xl items-center justify-center mr-4 ${isCurrentChapter ? "bg-blue-600" : "bg-slate-100"}`}
                      >
                        <Text
                          className={`font-black text-sm ${isCurrentChapter ? "text-white" : "text-slate-400"}`}
                        >
                      ...
                      <View className="flex-1 pr-4">
                        <View className="flex-row items-center gap-2 mb-0.5">
                          <Text
                            className={`font-bold text-sm flex-1 ${isCurrentChapter ? "text-blue-900" : "text-slate-800"}`}
                            numberOfLines={1}
                          >
                            {chapter.title}
                          </Text>
                          {isCurrentChapter && (
                            <View className="bg-blue-200 px-1.5 py-0.5 rounded">
                              <Text className="text-[9px] font-bold text-blue-800 uppercase tracking-widest">
                                Reading
                              </Text>
                            </View>
                          )}
                      ...
                      {chapter.is_locked ? (
                        ...
                        <View className="flex-row items-center gap-3">
                          ...
                          <Ionicons
                            name="play-circle"
                            size={28}
                            color={isCurrentChapter ? "#2563EB" : "#e2e8f0"}
                          />
                        </View>
                      )}
                    </Pressable>
        ...
        <View className="absolute bottom-8 right-6 z-30 shadow-lg">
          ...
          >
            ...
            <Text className="text-white font-black text-sm uppercase tracking-wider">
              {readingProgress
                ? `Ch. ${readingProgress.chapters?.sequence_number || ""}`
                : "Read"}
            </Text>
          </Pressable>
        </View>
