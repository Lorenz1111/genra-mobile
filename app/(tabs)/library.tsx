...
export default function LibraryScreen() {
  ...
  const fetchLibraryData = async () => {
    try {
      ...
      const { data: historyData, error: historyError } = await supabase
        ...
        .select(
          `updated_at, chapter_id, book_id, books (*), chapters (sequence_number, title)`,
        )
        .eq("user_id", user.id)
  ...
  const handleDeleteHistory = async (bookId: string) => {
    Alert.alert(
      "Remove History",
      "Are you sure you want to remove this book from your history?",
      [
        ...
        >
          <Image
            source={{
              uri:
                book.cover_url ||
                "https://placehold.co/150x200/ffffff/94a3b8.png?text=No+Cover",
            }}
            className="w-16 h-24 rounded-lg bg-slate-200"
            resizeMode="cover"
          />
          ...
          <View className="flex-1 ml-4 justify-center">
            ...
            <Text className="text-blue-600 font-semibold text-xs mb-2">
              Ch. {chapter?.sequence_number || "?"}
            </Text>
          ...
          <Pressable
            onPress={() =>
              item.chapter_id && router.push(`/book/read/${item.chapter_id}`)
            }
      ...
      <View className="px-6 pt-6 flex-1">
        ...
        <View className="flex-row bg-slate-100 p-1 rounded-full mb-4">
          <Pressable
            ...
            style={{
              backgroundColor:
                activeTab === "history" ? "white" : "transparent",
            }}
          >
            <Text
              className={`font-bold text-sm ${activeTab === "history" ? "text-blue-600" : "text-slate-500"}`}
            >
              History
            </Text>
          </Pressable>
          <Pressable
            ...
            style={{
              backgroundColor:
                activeTab === "bookmarks" ? "white" : "transparent",
            }}
          >
            <Text
              className={`font-bold text-sm ${activeTab === "bookmarks" ? "text-blue-600" : "text-slate-500"}`}
            >
              Saved
            </Text>
          </Pressable>
          <Pressable
            ...
            style={{
              backgroundColor:
                activeTab === "downloads" ? "white" : "transparent",
            }}
          >
            <Text
              className={`font-bold text-sm ${activeTab === "downloads" ? "text-blue-600" : "text-slate-500"}`}
            >
              Downloads
            </Text>
          </Pressable>
        ...
        {activeTab === "bookmarks" && bookmarkedBooks.length > 0 && (
          <View className="flex-row justify-end mb-4">
            <Pressable
              onPress={() =>
                setSortOption(sortOption === "recent" ? "az" : "recent")
              }
                ...
                ListEmptyComponent={
                  <View className="mt-20 items-center px-8">
                    ...
                    <Text className="text-slate-500 text-sm text-center">
                      Start reading some stories and they will magically appear
                      here.
                    </Text>
                  </View>
                }
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <View className="w-full aspect-[2/3] rounded-xl bg-slate-200 border border-slate-100 overflow-hidden mb-2 shadow-sm">
                      <Image
                        source={{
                          uri:
                            item.cover_url ||
                            "https://placehold.co/600x900/ffffff/94a3b8.png?text=No+Cover",
                        }}
                    ...
                    <Text
                      className="text-slate-800 font-bold text-xs leading-4 mb-0.5"
                      numberOfLines={2}
                    >
                    ...
                    <View className="flex-row items-center justify-between mt-1">
                      <View className="flex-row items-center gap-0.5">
                        ...
                        >
                          {(item as any).rating
                            ? Number((item as any).rating).toFixed(1)
                            : "0.0"}
                        </Text>
                      </View>
                      <View className="flex-row items-center gap-0.5">
                ...
                ListEmptyComponent={
                  <View className="mt-20 items-center px-8">
                    ...
                    <Text className="text-slate-500 text-sm text-center">
                      Go explore and bookmark the stories you love so you
                      won&#39;t lose them!
                    </Text>
                  </View>
                }
            ...
            {activeTab === "downloads" && (
              <View className="flex-1 items-center mt-20 px-8">
                ...
                <Text className="text-slate-500 text-sm text-center mb-8">
                  The ability to download chapters and read without an internet
                  connection is coming in the next update!
                </Text>
                <Pressable
                  onPress={() =>
                    Alert.alert(
                      "Coming Soon",
                      "We're working hard to bring you offline reading. Stay tuned!",
                    )
                  }
