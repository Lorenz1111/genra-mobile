  ...
  if (!books || books.length === 0)
    return (
      <View className="mb-8 px-6">
        <Text
          className="text-xl font-bold text-slate-900 mb-2 flex-1"
          numberOfLines={1}
        >
          {title}
        </Text>
  ...
  return (
    <View className="mb-8">
      <View className="flex-row justify-between items-center px-6 mb-3">
        <Text
          className="text-xl font-bold text-slate-900 pr-2 flex-1"
          numberOfLines={1}
        >
          {title}
        </Text>
...
export default function HomeScreen() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("avatar_url, coins, is_verified")
        .eq("id", user.id)
        .single();
      if (profile) {
      ...
      const myInterests =
        userPrefs?.map((up: any) => up.genres?.name).filter(Boolean) || [];
      ...
      setSliderGenres(
        Array.from(new Set(["For You", ...myInterests, ...allGenreNames])),
      );
      ...
      const { data: booksData, error } = await supabase
        .from("books")
        .select(
          `*, profiles(full_name), book_genres(genres(name)), chapters(sequence_number)`,
        )
        .eq("status", "approved")
      ...
      if (!error && booksData) {
        ...
        const topRated = [...formattedBooks].sort(
          (a, b) => (Number(b.rating) || 0) - (Number(a.rating) || 0),
        );
        setTopRatedBooks(topRated.slice(0, 10));

        const trending = [...formattedBooks].sort(
          (a, b) => (b.views_count || 0) - (a.views_count || 0),
        );
        setTrendingBooks(trending.slice(0, 10));

        const fresh = [...formattedBooks].sort(
          (a, b) =>
    ...
    if (selectedGenre === "For You") {
      if (interests.length > 0) {
        const matchedBooks = allBooks.filter((b) =>
          b.genreList.some((g: string) => interests.includes(g)),
        );
        setDynamicGenreBooks(
          matchedBooks.length > 0
            ? matchedBooks.slice(0, 10)
            : allBooks.slice(0, 10),
        );
      } else {
      ...
      const filtered = allBooks.filter((b) =>
        b.genreList.includes(selectedGenre),
      );
  ...
  const showPremiumAlert = () =>
    Alert.alert(
      "Coming Soon 🚀",
      "GenrA Premium subscriptions are currently in development. Stay tuned!",
    );
  const showCoinAlert = () =>
    Alert.alert(
      "Coming Soon 🪙",
      "Coin top-ups and daily rewards will be available in the next update!",
    );
    ...
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      ...
      >
        ...
        <View className="flex-row justify-between items-center px-6 pt-2 pb-6">
          ...
          >
            <View
              className={`w-2 h-2 rounded-full mr-2 ${isPremium ? "bg-amber-500" : "bg-slate-400"}`}
            />
            <Text className="text-slate-800 font-bold text-sm">
              {isPremium ? "GenrA Premium" : "Free Tier"}
            </Text>
            <Ionicons
              name="chevron-down"
              size={14}
        ...
        {continueReading && (
          <View className="px-6 mb-8">
            ...
            <Pressable
              ...
              onPress={() =>
                router.push(`/book/read/${continueReading.chapter_id}`)
              }
            >
              <Image
                source={{
                  uri:
                    continueReading.books.cover_url ||
                    "https://via.placeholder.com/150x200",
                }}
                className="w-20 h-28 rounded-lg bg-gray-700 border border-slate-700"
                resizeMode="cover"
              />
              <View className="flex-1 ml-4 justify-center">
                <Text
                  ...
                  numberOfLines={1}
                >
                  {continueReading.books.title}
                </Text>
                <Text
                  className="text-slate-400 text-sm mb-4 font-medium"
                  numberOfLines={1}
                >
                  Ch. {continueReading.chapters.sequence_number}:{" "}
        ...
        <View className="mb-6">
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <Pressable
                  onPress={() => handleGenreClick(item)}
                  className={`mr-2 px-4 py-2 rounded-full border ${isActive ? "bg-slate-900 border-slate-900" : "bg-white border-slate-200"}`}
                >
                  <Text
        ...
        <HorizontalBookList
          ...
          books={dynamicGenreBooks}
          onBookPress={handleBookPress}
          emptyMessage={`No books found for ${activeGenre} yet.`}
        />

        {/* TRENDING RANKED */}
        <HorizontalBookList
          title="Trending on GenrA"
          books={trendingBooks}
          onBookPress={handleBookPress}
          isRanked={true}
        />
          ...
          <View className="px-6 mb-8 mt-2">
            ...
            >
              <Image
                source={{
                  uri:
                    bookOfTheWeek.cover_url ||
                    "https://via.placeholder.com/150x200",
                }}
                className="w-24 h-36 rounded-lg bg-slate-200"
                resizeMode="cover"
              />
              <View className="flex-1 ml-4">
                <View className="bg-amber-100 self-start px-2 py-0.5 rounded text-amber-800 mb-1 border border-amber-200">
                ...
                <Text className="text-xs text-slate-500 font-medium mb-2">
                  by {bookOfTheWeek.profiles?.full_name || "Unknown Author"}
                </Text>
                <Text
                  className="text-sm text-slate-600 mb-2 leading-5"
                  numberOfLines={3}
                >
          ...
          >
            <View className="flex-1 pr-4">
              ...
              <Text className="text-blue-100 text-xs leading-4">
                Get 500 free coins on your first top-up and support your
                favorite authors.
              </Text>
            </View>
            <View className="bg-white px-5 py-2.5 rounded-full">
              <Text className="text-blue-600 font-black text-xs uppercase tracking-wider">
                Top Up
              </Text>
            </View>
          </Pressable>
