...
export default function SearchScreen() {
            ...
            if (!authorsMap.has(book.author_id)) {
              authorsMap.set(book.author_id, {
                ...
                avatar:
                  ...
                  `https://ui-avatars.com/api/?name=${book.profiles.full_name}&background=cbd5e1&color=fff`,
              });
            }
          }
        });
  ...
  const handleSearch = (text: string) => {
    ...
    const filtered = allBooks.filter((book) => {
      ...
      const authorMatch = book.profiles?.full_name
        ?.toLowerCase()
        .includes(searchLower);
      const genreMatch = book.genreList.some((genre: string) =>
  ...
  const saveToHistory = async (searchTerm: string) => {
    ...
    setRecentSearches((prev) => {
      ...
      AsyncStorage.setItem("search_history", JSON.stringify(newHistory)).catch(
        (err) => console.error("Error saving history:", err),
      );
  ...
  return (
    <SafeAreaView className="flex-1 bg-white pt-4" edges={["top"]}>
      <View className="px-6 mb-4">
        ...
        <Text className="text-slate-500 mt-1 text-sm font-medium mb-5">
        ...
        {loading ? (
          ...
          >
            <View className="mt-2 mb-8">
              <Text className="px-6 text-slate-800 font-bold text-base mb-3">
                Browse Genres
              </Text>
              {dbGenres.length > 0 ? (
                <FlatList
                ...
                <Text className="px-6 text-slate-400 italic">
                  Loading genres...
                </Text>
              )}
            </View>
                ...
                <View className="flex-row flex-wrap gap-2">
                  {recentSearches.map((searchItem, index) => (
                    ...
                    >
                      ...
                      <Text className="ml-1.5 text-slate-600 font-medium text-sm">
                        {searchItem}
                      </Text>
                    </Pressable>
                  ))}
            ...
            {trendingAuthors.length > 0 && (
              <View className="mb-8">
                <Text className="px-6 text-slate-800 font-bold text-base mb-4">
                  Trending Authors
                </Text>
                <FlatList
                  horizontal
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      ...
                      />
                      <Text
                        className="text-xs font-bold text-slate-800 text-center"
                        numberOfLines={2}
                      >
                        {item.name}
                      </Text>
